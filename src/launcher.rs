...
async fn run_enclave(sess: &Session) {
    ...
    channel.exec("nitro-cli run-enclave --cpu-count 2 --memory 4500 --eif-path startup.eif --debug-mode").unwrap();
...
async fn launch_instance(client: &Client, key_pair_name: &str) -> String {
    ...
    let enclave_options = aws_sdk_ec2::model::EnclaveOptionsRequest::builder().set_enabled(Some(true)).build();
    let ebs = aws_sdk_ec2::model::EbsBlockDevice::builder().volume_size(25).build();
    let block_device_mapping = aws_sdk_ec2::model::BlockDeviceMapping::builder().set_device_name(Some("/dev/sda1".to_string())).set_ebs(Some(ebs)).build();
    let name_tag = aws_sdk_ec2::model::Tag::builder().set_key(Some("Name".to_string())).set_value(Some("tester".to_string())).build();
    let managed_tag = aws_sdk_ec2::model::Tag::builder().set_key(Some("managedBy".to_string())).set_value(Some("marlin".to_string())).build();
    let tags = aws_sdk_ec2::model::TagSpecification::builder().set_resource_type(Some(aws_sdk_ec2::model::ResourceType::Instance)).tags(name_tag).tags(managed_tag).build();
    ...
    match resp {
        Ok(res) => {
            for instance in res.instances().unwrap_or_default() {
                println!("Instance launched - ID: {}", instance.instance_id().unwrap());
